...
def build_report() -> Document:
    ...
    doc = Document()
    configure(doc, "Corporate Financial Deterioration | Integrated Financial Risk Research Analysis")
    doc.sections[0].footer.paragraphs[0].text = (
        "Integrated financial risk analysis | Evidence levels remain separate"
    )
    cover(
        doc,
    ...
    add_table(doc, ["Phase 1 limitation", "Phase 2 response"], [
        ...
        ["Limited case interpretation", "Plain-language alert explanations, uncertainty indicators, reviews of correct alerts, false alerts, and missed deteriorations, plus prompts for analyst follow-up"],
    ], [3350, 6010])
    ...
    figure(doc, FIGURES / "horizon_tradeoff.png", "Figure 1. The two-quarter label reduces queue size but loses ranking quality, precision, and warning time.")
    ...
    add_table(doc, ["Test", "Population", "Method", "Purpose"], [
        ["A", "Original 60 companies", "Phase 1", "Establish the common-period baseline."],
        ["B", "Original 60 companies", "Phase 2", "Compare methodology with A while holding the population constant."],
        ["C", "Expanded 117 companies", "Phase 1", "Measure population expansion under the Phase 1 methodology."],
        ["D", "Expanded 117 companies", "Phase 2", "Compare methodology with C while holding the expanded population constant."],
    ], [1100, 2000, 1600, 4660])
    ...
    bullet(doc, "Bridge endpoints: four-quarter PR-AUC, Brier score, calibration, recall, precision, and alert workload under both the fixed 20% alert-rate comparison and the recall-first operating policy.")
    bullet(doc, "Primary prospective endpoint: four-quarter PR-AUC, with Phase 1's 0.397 benchmark treated as historical context rather than a guaranteed target.")
