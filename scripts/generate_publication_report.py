...
def build() -> Path:
    ...
    add_body(doc, "Only a minority of observations are deterioration events, so simple accuracy can be misleading. PR-AUC summarizes how well the model separates deterioration cases while focusing on the less common event class. Recall is the percentage of actual events detected. Precision is the percentage of alerts that became events. Top-decile lift compares the event rate among the highest-risk 10% with the overall event rate. Brier score measures the average squared error of the probabilities, and calibration checks whether a stated risk such as 30% occurs about 30% of the time. No single measure answers every business question.")
    ...
    add_body(doc, "Primary internal references: README.md; docs/point_in_time_policy.md; docs/label_specification.md; docs/model_card.md; docs/modeling_lineage.md; and the stage execution records. External data originate from SEC EDGAR and FRED/ALFRED under their respective public usage policies.")
