...
SOURCE_DATA = ROOT / "reports/source_data"
...
def font(
    run, size: float = 10.5, bold: bool = False, color: str = "202124", italic: bool = False
) -> None:
...
def make_figures(
    watch: pd.DataFrame, model: pd.DataFrame, portfolio: pd.DataFrame, history: pd.DataFrame
) -> None:
    ...
    counts = (
        watch.groupby(["sector", "risk_band"]).size().unstack(fill_value=0).reindex(columns=bands)
    )
    ax = counts.plot(
        kind="bar",
        stacked=True,
        color=["#5B8FF9", "#F6BD16", "#E8684A", "#B42318"],
        figsize=(8.2, 4.2),
    )
    ...
    plt.savefig(FIG / "risk_band_by_sector.png", dpi=220)
    plt.close()
    ...
    ax.set(
        title="Highest latest deterioration-risk estimates",
        xlabel="Estimated deterioration risk (%)",
        ylabel="",
    )
    ax.axvline(
        watch.loc[watch.alert, "threshold"].median() * 100,
        color=f"#{NAVY}",
        ls="--",
        lw=1.2,
        label="Median alert threshold",
    )
    ax.legend(frameon=False, loc="lower right")
    plt.tight_layout()
    plt.savefig(FIG / "top_risk_companies.png", dpi=220)
    plt.close()

    metrics = portfolio.set_index("sector")[
        ["median_interest_coverage_x", "median_free_cash_flow_margin", "median_debt_to_assets"]
    ]
    fig, axes = plt.subplots(1, 3, figsize=(8.4, 3.5))
    labels = [
        ("median_interest_coverage_x", "Interest coverage", "x", 1),
        ("median_free_cash_flow_margin", "FCF margin", "%", 100),
        ("median_debt_to_assets", "Debt/assets", "%", 100),
    ]
    ...
    fig.suptitle(
        "Sector-level financial condition at the latest evaluated snapshot", fontweight="bold"
    )
    plt.tight_layout()
    plt.savefig(FIG / "sector_kpi_comparison.png", dpi=220)
    plt.close()
    ...
    ax.set(
        title="Locked-holdout discrimination and alert-quality metrics",
        xlabel="",
        ylabel="Metric value",
        ylim=(0, 0.7),
    )
    ...
    plt.tight_layout()
    plt.savefig(FIG / "holdout_sector_performance.png", dpi=220)
    plt.close()
    ...
    ax.plot(
        champ["fold_id"].astype(str), champ["PR_AUC"], marker="o", color=f"#{NAVY}", label="PR-AUC"
    )
    ax.plot(
        champ["fold_id"].astype(str),
        champ["Brier_score"],
        marker="o",
        color=f"#{AMBER}",
        label="Brier score",
    ...
    ax.set(
        title="Champion out-of-fold performance across expanding-window folds",
        xlabel="Temporal fold",
        ylabel="Metric value",
    )
    ax.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(FIG / "temporal_fold_stability.png", dpi=220)
    plt.close()
    ...
    for ax, (column, title, unit, scale) in zip(axes, trend_specs, strict=True):
        for sector_name, sector_data in trend.groupby("sector"):
            ax.plot(
                sector_data["fiscal_year"],
                sector_data[column] * scale,
                marker="o",
                ms=3,
    ...
    plt.tight_layout()
    plt.savefig(FIG / "sector_kpi_time_series.png", dpi=220)
    plt.close()

    seasonal = (
        history.groupby(["fiscal_quarter", "sector"])[
            ["interest_coverage_ttm", "free_cash_flow_margin_ttm", "total_debt_to_assets"]
        ]
        .median()
        .reset_index()
    ...
    for ax, (column, title, unit, scale) in zip(axes, trend_specs, strict=True):
        for sector_name, sector_data in seasonal.groupby("sector", observed=True):
            ax.plot(
                sector_data["fiscal_quarter"].astype(str),
                sector_data[column] * scale,
                marker="o",
                color=colors[sector_name],
            )
    ...
    plt.tight_layout()
    plt.savefig(FIG / "sector_kpi_fiscal_quarter.png", dpi=220)
    plt.close()
...
def build() -> Path:
    ...
    portfolio = pd.read_parquet(SOURCE_DATA / "phase1_portfolio_snapshot.parquet")
    watch = pd.read_parquet(SOURCE_DATA / "phase1_company_watchlist.parquet")
    model = pd.read_parquet(SOURCE_DATA / "phase1_model_performance.parquet")
    history = pd.read_parquet(SOURCE_DATA / "phase1_company_history.parquet")
    ...
    font(
        fp.add_run(
            "Independent analytical research | Not investment advice or a default-probability model"
        ),
        8,
        color=GRAY,
    )
    ...
    font(
        p.add_run(
            "A point-in-time, experimentally governed study of debt-service capacity across Consumer Discretionary and Utilities issuers"
        ),
        13,
        color=BLUE,
    )
    add_table(
        ...
        [
            [
                "Expanding-window validation with locked holdout",
                "60 issuers | 3,150 company-quarters",
                "Analyst prioritization and surveillance",
            ]
        ],
        [3120, 3120, 3120],
    )
    ...
    add_table(
        ...
        [
            [
                "H1: combining signals improves ranking",
                "Compare planned feature groups using time-ordered, out-of-sample PR-AUC",
                "Improvement must persist across time periods and sectors",
            ],
            [
                "H2: forecasts add forward-looking information",
                "Add 1Q/4Q KPI forecasts built without future information",
                "Added value must appear before the final test period",
            ],
            [
                "H3: performance is regime- and sector-sensitive",
                "Report fold, sector, calibration, and final-holdout slices",
                "No pooled metric may conceal weak subgroups",
            ],
        ],
        [1800, 4200, 3360],
    )
    ...
    add_table(
        ...
        [
            [
                ...
                "Concept mapping plus fiscal-period standardization",
                "Issuer taxonomies and fiscal calendars are nonuniform; economic definitions must be aligned before cross-sectional comparison.",
            ],
            [
                "Point-in-time join",
                "Use each value only after its public availability date",
                "Prevents the model from learning from late filings, later revisions, or future outcomes.",
            ],
            [
                "Peer context",
                "Within-sector percentile ranks",
                "Financial ratios are structurally sector-dependent; peer ranks reduce misleading level comparisons.",
            ],
            [
                "Missing values",
                "Preserve valid blanks; estimate replacements within each training window",
                "Replacing every blank with zero creates false financial values, while using the full dataset would leak future information.",
            ],
        ],
        [1800, 2700, 4860],
    )
    ...
    add_figure(
        doc,
        FIG / "sector_kpi_fiscal_quarter.png",
        "Figure 4. Standardized fiscal-quarter medians. Trailing-twelve-month construction intentionally smooths quarter-specific noise.",
    )
    add_body(
        doc,
    ...
    add_body(
        doc,
        "Methodological implication. The weak quarter pattern is expected because the ratios use trailing-twelve-month values, which combine four quarters and reduce seasonal noise. The stronger empirical feature is persistence: recent KPI levels contain substantial information about the next quarter. This explains why a random-walk forecast was difficult to beat for one-quarter cash-flow margin and coverage, while a local-level model was more suitable for gradual leverage changes and four-quarter horizons. Sector-specific baselines, changes from each company's own history, and macro conditions are therefore more defensible than broad seasonal adjustments.",
        "Methodological implication.",
    )

    add_heading(
        doc, "5. Forecasting methodology: matching model structure to temporal financial data"
    )
    add_body(
        doc,
    ...
    add_table(
        ...
        [
            [
                "Random walk",
                "Many accounting ratios are persistent and difficult to beat out of sample.",
                "Mandatory naive benchmark; prevents complexity from receiving credit for persistence.",
            ],
            [
                "Drift",
                "Allows gradual balance-sheet or margin trajectories.",
                "Tests whether a simple secular trend adds value over the last observation.",
            ],
            [
                "Local level",
                "Estimates an underlying financial level while allowing quarterly reporting noise.",
                "Useful for noisy ratios when the company's true condition changes gradually.",
            ],
            [
                "Local linear trend",
                "Allows both the financial level and its direction of change to move over time.",
                "Useful for gradual operating or leverage trends, but can be unstable with short histories.",
            ],
            [
                "Dynamic regression model",
                "Adds economic variables to a time-series model whose financial level changes over time.",
                "Tests whether rates and business-cycle conditions improve forecasts.",
            ],
        ],
        [1700, 4000, 3660],
    )
    ...
    add_body(
        doc,
        "How information was added to the model. Before testing began, the research plan specified five groups of information to examine: (1) each company's current and past financial ratios, (2) its position relative to companies in the same sector, (3) forecasts of where its ratios may be heading, built without using future information, (4) economic conditions such as interest rates and business-cycle measures, and (5) a small number of sector-specific combinations.",
        "How information was added to the model.",
    )
    add_body(
        doc,
        "Why test the groups separately? Adding one group at a time shows whether it improves predictions and reduces cherry-picking. Peer comparisons use the correct sector benchmark; forecasts describe direction; economic variables capture shared pressures; and a few sector combinations allow the same signal to matter differently in each industry without testing thousands of unplanned relationships.",
        "Why test the groups separately?",
    )
    add_body(
        doc,
        "Why accuracy alone is misleading. Deterioration occurred in about 21% of observations. Predicting “no deterioration” every time would therefore appear roughly 79% accurate while finding no important cases. PR-AUC measures how well actual deterioration cases are ranked ahead of non-events. Recall is the share of actual cases detected; precision is the share of alerts that became cases. Top-decile lift compares the highest-risk 10% with the portfolio average. Brier score measures probability error, while calibration checks whether predicted risks match observed frequencies. The measures answer different questions and should be reviewed together.",
    ...
    add_body(
        doc,
        "Overall, the study supports the value of combining several types of information and confirms that model performance depends on sector and time period. Forecast features appear promising but require further testing before they should be treated as a consistently reliable improvement.",
    )
    ...
    add_figure(
        doc,
        FIG / "top_risk_companies.png",
        "Figure 7. Highest latest risk estimates; company rankings are analytical screening outputs, not investment recommendations.",
    )
    add_body(
        doc,
    ...
    add_table(
        ...
        [
            [
                "Environment",
                "Python 3.12 project with locked dependencies and deterministic commands",
            ],
            [
                "Lineage",
                "SEC/FRED acquisition manifests, checksums, versioned configurations, and point-in-time policies",
            ],
            [
                "Model governance",
                "OOF development predictions, fold-local transformations, frozen champion, one-time holdout",
            ],
            [
                "Analysis reconciliation",
                "60 unique companies, 3,150 unique decision records, and 93 model-performance records",
            ],
            [
                "Release evidence",
                "Certified publication tables, figures, source documentation, and automated tests",
            ],
        ],
        [2600, 6760],
    )
    ...
    add_body(
        doc,
        "Disclaimer. This report is an analytical portfolio project for research and demonstration. It is not investment advice, a credit rating, a bankruptcy/default model, or a substitute for audited financial statements and professional judgment.",
    )
