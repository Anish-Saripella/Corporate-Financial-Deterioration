# Documentation index

Use this page to navigate the detailed technical record. Recruiters and first-time readers should
begin with the root [project overview](../README.md) and the [published reports](../reports/publication/README.md).

## Project and research design

- [Project charter](project_charter.md)
- [Phase 1 architecture](architecture.md)
- [Phase 2 scope and sampling](phase2_scope_and_sampling.md)
- [Phase 2 methodology](phase2_methodology.md)
- [Phase 2 model optimization and ensemble methodology](phase2_model_optimization_methodology.md)
- [Assumptions and limitations](assumptions_and_limitations.md)

## Data and point-in-time controls

- [Data-source register](data_sources.md)
- [Company-universe policy](universe_policy.md)
- [Point-in-time and fiscal-period policy](point_in_time_policy.md)
- [Data card](data_card.md)
- [Feature and KPI dictionary](feature_dictionary.md)
- [Deterioration-label specification](label_specification.md)

## Modeling, validation, and governance

- [Modeling lineage](modeling_lineage.md)
- [Time-aware validation plan](validation_plan.md)
- [Phase 1 model card](model_card.md)
- [Architecture decisions](decisions/)

## Reproducibility and execution evidence

- [Phase 1 reproducibility standard](reproducibility.md)
- [Phase 2 reproducibility guide](phase2_reproducibility.md)
- [Stages 0–7 execution record](stages_0_7_execution.md)
- [Stages 8–12 execution record](stages_8_12_execution.md)
- [Stages 13–16 execution record](stages_13_16_execution.md)
- [Phase 1 chronological implementation record](phase1_implementation_plan.md)

## Portfolio material

- [Detailed project case study](case_study.md)
- [Resume and interview summary](resume_summary.md)
